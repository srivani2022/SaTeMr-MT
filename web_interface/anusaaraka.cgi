...
require "$GlblVar::SCLINSTALLDIR/converters/convert.pl";
  ...
  if($GlblVar::LOG eq "true") {
      ...
      if ($tlang eq "Hindi") { $prog = "anu_skt_hnd.sh";}
      elsif ($tlang eq "Telugu") { $prog = "anu_skt_tlg.sh";}
      elsif ($tlang eq "Marathi") { $prog = "anu_skt_mrt.sh";}
	 ...
	 $cmd = "QUERY_STRING=\"lex=MW\&cache=f\&st=t\&us=f\&font=deva\&cp=t\&text=$sentences\&t=WX\&topic=\&mode=f&pipeline=t\" $GlblVar::CGIDIR/$GlblVar::HERITAGE_CGI > /tmp/yyy ; tail -1 /tmp/yyy | $GlblVar::SCLINSTALLDIR/MT/prog/Heritage_morph_interface/Heritage2anusaaraka_morph.sh $GlblVar::SCLINSTALLDIR $GlblVar::TFPATH $pid";
         ...
         system("$GlblVar::TIMEOUT $GlblVar::SCLINSTALLDIR/MT/prog/shell/$prog.sh $GlblVar::CGIDIR/scl tmp_in${pid}/in$pid $GlblVar::TFPATH hi $display $morph Full $text_type 2> $GlblVar::TFPATH/tmp_in$pid/err$pid");
         }  else {
         ...
         close (TMP);
          ...
          $cmd = "$GlblVar::TIMEOUT $GlblVar::SCLINSTALLDIR/MT/prog/shell/$prog $GlblVar::CGIDIR/scl tmp_in${pid}/in$pid $GlblVar::TFPATH hi $script $morph $parse $text_type 2>> $GlblVar::TFPATH/tmp_in$pid/err$pid;";
